...
// GET /api/gallery/categories/:id/images — Public
export async function GET(
  ...
  try {
    ...
    const category = await GalleryCategory.findById(id).select("_id");

    if (!category) {
      return notFoundResponse("Gallery category not found");
    }

    const images = await GalleryImage.find({ categoryId: id }).sort({
      createdAt: -1,
    });

    return successResponse(images, "Gallery images retrieved successfully");
  } catch {
    return errorResponse("Failed to retrieve gallery images");
  }
}

// POST /api/gallery/categories/:id/images — Protected
// Accepts multipart/form-data with one or more files under field name
// "images" (or a single file under "image").
export async function POST(
  ...
  try {
    ...
    // Optional per-file metadata: JSON array aligned with the file order,
    // e.g. metadata=[{"title":"Award","description":"..."}]
    ...
    if (typeof rawMetadata === "string" && rawMetadata.trim() !== "") {
      ...
      // Fallback: plain title/description fields applied to all uploaded files
    ...
    const folder = `arisa-nutrition/gallery/${category.slug}`;
